...
**Claude Code** is Claude running directly in your terminal or IDE — not a chat window, but an agent with real access to your files, your tests, and your git history. It can read code, edit it, run commands, and chain multiple steps together without you in the loop.
...
Together they mean Claude is not guessing what your project looks like. It can see it, run it, and verify its own work. The result: instead of a chatbot that needs constant nudging, you get an autonomous coding agent that iterates until the job is done.

---

## The 7 habits that make it work

These are ordered deliberately — start simple, build up. By habit 7 you will understand how to hand a task to Claude and walk away.
...
- src/auth/           (handled separately)
- config/database.py  (production credentials)
- any file in tests/  (tests define the contract)
- requirements.txt    (dependencies are locked)
```

When Claude touches something explicitly forbidden, you catch it immediately.
...
Do not save all testing for the last step:
...
Some tasks cannot be solved in a single pass — a threshold that needs tuning, a bug that requires multiple approaches. Normally you would send follow-up messages for each iteration, pasting in errors, losing context, repeating yourself.

Instead, write the loop directly into the prompt:
...
2. All tests pass → stop, go to Final Steps.
   Still failing AND attempts < 3 → identify the specific failure,
   make one targeted fix, repeat from step 1.
   Attempts = 3 AND still failing → stop, report what failed,
   go to Final Steps.
...
## Iteration loop
1. Run: [test command]
2. Target met → Final Steps.
   Not met AND attempts < [N] → fix the failure and repeat.
   Attempts = [N] → report status and go to Final Steps.
